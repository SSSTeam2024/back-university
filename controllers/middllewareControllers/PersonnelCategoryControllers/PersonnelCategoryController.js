...
const getPersonnelCategoriesOfAllFaculties = async (req, res) => {
  try {
    const faculties = await facultyService.getFacultiesService();

    let totalCategories = [];

    let totalCleanCategories = [];

    for (const faculty of faculties) {
      const FACULTY_API = faculty.server_domain_name;

      const categories = await axios.get(
        `${FACULTY_API}/categorie-personnel/get-all-categorie-personnel`
      );

      totalCategories = totalCategories.concat(categories.data);
    }

    totalCleanCategories = removeDuplicates(totalCategories, "categorie_fr");

    if (!totalCleanCategories) {
      return res.status(404).send("categories not found");
    }
    res.json(totalCleanCategories);
  } catch (error) {
    console.error(error);
    res.status(500).send(error.message);
  }
...
module.exports = {
  ...
  getPersonnelCategoriesOfAllFaculties
  // getAllFaculties,
